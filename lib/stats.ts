...
export async function getPlatformStats(): Promise<PlatformStats> {
  try {
    ...
    //("Number of unique referrers:", uniqueReferrers);
    ...
    //("Ideas shared query:", { ideasShared, ideasSharedError });
    ...
    //("Reach data query:", { reachData, reachError });

    // const livesReached =
    //   reachData?.reduce((sum, idea) => {
    //     const reach = Number(idea.reach) || 0;
    //     //("Processing idea reach:", { ideaId: idea.id, reach });
    //     return sum + reach;
    //   }, 0) || 0;

    //("Calculated lives reached:", reachData);
